...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const visibleCount = 4; // сколько видно картинок одновременно
    let index = 0;

    function updateGallery() {
        const imageWidth = images[0].offsetWidth + 20; // 20px — gap между картинками
        const maxIndex = imageCount - visibleCount;
        const offset = -index * imageWidth;
        gallery.style.transform = `translateX(${offset}px)`;

        // Деактивировать кнопку, если достигли края
